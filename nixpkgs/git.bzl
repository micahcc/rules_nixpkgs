...
def git_repo(ctx, directory, *, shallow_since, tag, branch, commit, remote, init_submodules, verbose, strip_prefix, recursive_init_submodules):
    ...
    if shallow_since:
        shallow = "--shallow-since=%s" % shallow_since
    else:
        shallow = "--depth=1"
    ...
    if commit:
        ...
        reset_ref = "tags/" + tag
        fetch_ref = "tags/" + tag + ":tags/" + tag
    elif branch:
        reset_ref = "origin/" + branch
        fetch_ref = branch + ":origin/" + branch
    ...
    ctx.report_progress("Cloning %s of %s" % (reset_ref, remote))
    if verbose:
        print("git.bzl: Cloning or updating %s repository %s using strip_prefix of [%s]" %
              (
                  " (%s)" % shallow if shallow else "",
                  ctx.name,
                  strip_prefix if strip_prefix else "None",
              ))

    _update(ctx, git_repo, remote = remote, commit = commit)
...
def _update(ctx, git_repo, *, remote, commit):
    ctx.delete(git_repo.directory)

    init(ctx, git_repo)
    add_origin(ctx, git_repo, remote)
    fetch(ctx, git_repo, commit = commit)
    reset(ctx, git_repo)
    clean(ctx, git_repo)
